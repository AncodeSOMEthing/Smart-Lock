...
int lastDoorState = 0; // Trạng thái cửa ban đầu là đóng
int buzzerPin = 8;
int servoPin = 3;
int resetPassPin = 2;
...
void setup() {
  ...
  // Đảm bảo cửa bắt đầu ở trạng thái đóng
  myServo.write(0);
}
    ...
    case '1': // Đổi mật khẩu 1
     ...
     case '2': // Đổi thẻ RFID Master
        ...
        if (oldPass == defaultPassword1) { // Xác thực bằng mật khẩu 1
          ...
          String newMasterUID = readUIDNum(); // Đọc UID từ thẻ mới
          if (newMasterUID != "") { // Nếu quét thành công thẻ
            UIDMaster = newMasterUID;
            Serial.print("[RFID] --> Master UID changed to: ");
            Serial.println(UIDMaster);
            lcd.clear();
            lcd.print("Master updated!");
            delay(delayTimeDefault);
          }
      } else { // Thực hiện khi nhập sai mật khẩu
        changePassAttempts--;
